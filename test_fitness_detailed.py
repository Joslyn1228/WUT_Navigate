    ...
    try:
        ...
        if login_response.status_code == 200:
            ...
            if "access_token" in login_result:
                ...
                print(f"开始运动状态码: {start_response.status_code}")
                start_result = start_response.json()
                print(f"响应: {json.dumps(start_result, ensure_ascii=False)}")
                ...
                if start_result.get("success") and start_result.get("data", {}).get("workout_id"):
                    ...
                    print(f"结束运动状态码: {end_response.status_code}")
                    end_result = end_response.json()
                    print(f"响应: {json.dumps(end_result, ensure_ascii=False)}")
                    ...
                    if end_result.get("success"):
                        ...
                        print(f"统计状态码: {stats_response.status_code}")
                        stats_result = stats_response.json()
                        print(f"响应: {json.dumps(stats_result, ensure_ascii=False)}")
                        ...
                        print(f"响应: {json.dumps(history_result, ensure_ascii=False)}")
                else:
                    print("❌ 运动开始失败")
            else:
