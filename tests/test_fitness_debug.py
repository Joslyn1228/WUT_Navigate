...
# 测试用用户
test_user = "1025181275@qq.com"
test_password = "Wut@2024"
    ...
    try:
        ...
        if login_response.status_code == 200:
            ...
            if "access_token" in login_result:
                ...
                print("开始运动响应:", json.dumps(start_result, ensure_ascii=False))
                ...
                if start_result.get("success") and start_result.get("data", {}).get("workout_id"):
                    ...
                    print("结束运动响应:", json.dumps(end_result, ensure_ascii=False))
                    ...
                    print("周统计数据:", json.dumps(stats_result, ensure_ascii=False))
                    
                    print("\n5. 获取运动统计(全部)...")
                    stats_all_response = requests.get(
                        BASE_URL + "/api/fitness/statistics?period=all",
                        headers=headers
                    )
                    print("统计状态码:", stats_all_response.status_code)
                    stats_all_result = stats_all_response.json()
                    print("累计统计数据:", json.dumps(stats_all_result, ensure_ascii=False))
                    ...
                    print("运动历史数据:", json.dumps(history_result, ensure_ascii=False))
                else:
                    print("运动开始失败")
            else:
